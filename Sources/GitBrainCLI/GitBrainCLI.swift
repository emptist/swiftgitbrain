...
struct GitBrainCLI {
    ...
    private static func handleInit(args: [String]) async throws {
        ...
        print("\nNext steps:")
        print("1. Open Trae at project root for Creator: trae .")
        print("2. Open Trae at GitBrain for Monitor: trae ./.GitBrain")
    }
    ...
    private static func handleSendHeartbeat(args: [String]) async throws {
        ...
        guard let status = HeartbeatStatus(rawValue: statusStr) else {
            throw CLIError.invalidArguments("Invalid status. Valid statuses: \(HeartbeatStatus.allCases.map { $0.rawValue }.joined(separator: ", "))")
        }
    ...
    private static func handleSendFeedback(args: [String]) async throws {
        ...
        let to = args[0]
        let feedbackTypeStr = args[1]
        let subject = args[2]
        let content = args[3]
        let relatedTaskId = args.count > 4 ? args[4] : nil
        
        guard let feedbackType = FeedbackType(rawValue: feedbackTypeStr) else {
            throw CLIError.invalidArguments("Invalid feedback_type. Valid types: \(FeedbackType.allCases.map { $0.rawValue }.joined(separator: ", "))")
        }
