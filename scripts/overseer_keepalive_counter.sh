...
log() {
    echo -e "${BLUE}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} OverseerAI Keep-Alive: $1"
}

log_success() {
    echo -e "${GREEN}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} OverseerAI Keep-Alive: $1"
}
...
log "Starting OverseerAI keep-alive loop (increment every ${INCREMENT_INTERVAL}s)..."
